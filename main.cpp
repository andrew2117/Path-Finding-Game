...
int main()
{
    ...
    Graphics player = Graphics(player_position, sf::Color::Green);

    int opponent_position[2] = {rows - 1, 0};
    Graphics opponent = Graphics(opponent_position, sf::Color::Red);

    // algorithm
    DijkstraAlgorithm algo = DijkstraAlgorithm(rows, columns, opponent_position, end_position, grid.getObstacles());
    std::vector<pi> shortest_path = algo.plan();
